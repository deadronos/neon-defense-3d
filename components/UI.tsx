...
const Tooltip = ({ text, className = "" }: { text: React.ReactNode, className?: string }) => (
  <div className={`absolute opacity-0 group-hover:opacity-100 transition-all duration-200 bg-black/95 border border-gray-700 text-gray-300 text-xs rounded px-2 py-1.5 pointer-events-none z-50 shadow-xl backdrop-blur-sm whitespace-nowrap -translate-y-1 group-hover:translate-y-0 ${className}`}>
    {text}
  </div>
);

export const UI = () => {
  const { gameState, startGame, resetGame, selectedTower, setSelectedTower, selectedEntityId, towers, upgradeTower, sellTower, setSelectedEntityId } = useGame();
  ...
  const selectedTowerEntity = selectedEntityId ? towers.find(t => t.id === selectedEntityId) : null;
      ...
      <div className="absolute top-0 w-full p-4 flex justify-between items-start pointer-events-auto">
        <div className="flex gap-4">
          <div className="group relative bg-gray-900/80 backdrop-blur border border-gray-700 px-4 py-2 rounded-lg flex flex-col items-center min-w-[100px] transition-all duration-300 hover:border-green-500/50 hover:bg-gray-800 hover:scale-105 cursor-default shadow-lg">
            ...
            <span className={`text-2xl font-bold ${gameState.lives < 10 ? 'text-red-500' : 'text-green-400'}`}>
              {gameState.lives}
            </span>
            <Tooltip text="If this reaches 0, the mission fails." className="top-full mt-2 left-1/2 -translate-x-1/2" />
          </div>
          <div className="group relative bg-gray-900/80 backdrop-blur border border-gray-700 px-4 py-2 rounded-lg flex flex-col items-center min-w-[100px] transition-all duration-300 hover:border-yellow-500/50 hover:bg-gray-800 hover:scale-105 cursor-default shadow-lg">
            ...
            <Tooltip text="Earn credits by destroying enemies." className="top-full mt-2 left-1/2 -translate-x-1/2" />
          </div>
          <div className="group relative bg-gray-900/80 backdrop-blur border border-gray-700 px-4 py-2 rounded-lg flex flex-col items-center min-w-[100px] transition-all duration-300 hover:border-cyan-500/50 hover:bg-gray-800 hover:scale-105 cursor-default shadow-lg">
             ...
             <Tooltip text="Enemies get stronger each wave." className="top-full mt-2 left-1/2 -translate-x-1/2" />
          </div>
        </div>
      </div>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected ? 'bg-gray-700 ring-2 ring-cyan-400 scale-105 -translate-y-2 shadow-cyan-500/20 shadow-lg' : 'hover:bg-gray-800 hover:-translate-y-2'}
                ...
                >
                  ...
                  />
                  <span className="text-xs font-bold text-white group-hover:text-cyan-300 transition-colors">{config.name}</span>
                  <span className={`text-xs transition-colors ${canAfford ? 'text-yellow-300' : 'text-gray-500'}`}>${config.cost}</span>
                  ...
                  <div className="absolute bottom-full mb-4 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-200 w-48 bg-black/95 p-3 rounded-lg text-xs text-left border border-gray-700 pointer-events-none shadow-xl z-20 backdrop-blur-sm">
                    ...
                    <div className="grid grid-cols-2 gap-1 text-gray-400 font-mono text-[10px]">
                      <span className="flex justify-between"><span>DMG:</span> <span className="text-white">{config.damage}</span></span>
                      <span className="flex justify-between"><span>SPD:</span> <span className="text-white">{(1/config.cooldown).toFixed(1)}/s</span></span>
                      <span className="flex justify-between"><span>RNG:</span> <span className="text-white">{config.range}</span></span>
                    </div>
                  </div>
                </button>
              );
           ...
           <div className="bg-gray-900/95 backdrop-blur border border-cyan-500/50 p-4 rounded-2xl flex gap-6 shadow-2xl min-w-[400px] animate-fade-in-up transition-all duration-300 hover:shadow-cyan-500/20 hover:border-cyan-400/80">
              ...
              <div className="flex flex-col gap-2">
                 <h2 className="text-xl font-bold text-white flex items-center gap-2">
                    <span className="w-4 h-4 rounded-full shadow-[0_0_10px_currentColor]" style={{ background: TOWER_CONFIGS[selectedTowerEntity.type].color, color: TOWER_CONFIGS[selectedTowerEntity.type].color }} />
                    {TOWER_CONFIGS[selectedTowerEntity.type].name} 
                    <span className="text-sm bg-gray-800 px-2 py-0.5 rounded text-cyan-300 border border-gray-700 shadow-inner">Lvl {selectedTowerEntity.level}</span>
                 </h2>
                        ...
                        return (
                          <>
                             <div className="text-gray-400 self-center relative group cursor-help border-b border-gray-700/50 hover:border-gray-500 border-dotted w-max">
                               Damage
                               <Tooltip text="Health reduction per hit" className="bottom-full mb-2 left-0" />
                             </div>
                             <div className="text-white font-mono bg-black/30 px-2 py-0.5 rounded border border-gray-800">
                               {current.damage.toFixed(0)} <span className="text-green-400 ml-1">→ {next.damage.toFixed(0)}</span>
                             </div>

                             <div className="text-gray-400 self-center relative group cursor-help border-b border-gray-700/50 hover:border-gray-500 border-dotted w-max">
                               Range
                               <Tooltip text="Max targeting distance" className="bottom-full mb-2 left-0" />
                             </div>
                             <div className="text-white font-mono bg-black/30 px-2 py-0.5 rounded border border-gray-800">
                               {current.range.toFixed(1)} <span className="text-green-400 ml-1">→ {next.range.toFixed(1)}</span>
                             </div>
                             ...
                             <div className="text-white font-mono bg-black/30 px-2 py-0.5 rounded border border-gray-800">
                               {(1/current.cooldown).toFixed(1)} <span className="text-green-400 ml-1">→ {(1/next.cooldown).toFixed(1)}/s</span>
                             </div>
                          </>
                        )
                    })()}
              ...
              <div className="flex flex-col justify-between gap-2 border-l border-gray-700 pl-6">
                 ...
                 <div className="flex gap-2">
                    ...
                    >
                        SELL
                        <Tooltip text="Refunds 70% of total value" className="bottom-full mb-2 left-1/2 -translate-x-1/2" />
                    </button>
                    ...
                    {(() => {
                       ...
                       return (
                          ...
                          >
                            ...
                            <span className={`text-xs ${canAfford ? 'text-yellow-300' : ''}`}>${nextStats.upgradeCost}</span>
