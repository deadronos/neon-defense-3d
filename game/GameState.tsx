...
import { EnemyEntity, GameState, ProjectileEntity, TowerEntity, TowerType, TileType, EffectEntity } from '../types';
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startGame = useCallback(() => {
    setGameState(prev => ({ ...prev, isPlaying: true, gameStatus: 'playing', lives: 20, money: 150, wave: 1 }));
    setEnemies([]);
    setTowers([]);
    setProjectiles([]);
    setEffects([]);
  ...
  const resetGame = useCallback(() => {
    setGameState(prev => ({ ...prev, isPlaying: false, gameStatus: 'idle', lives: 20, money: 150, wave: 1 }));
    setEnemies([]);
    setTowers([]);
    setProjectiles([]);
    setEffects([]);
  ...
  const isValidPlacement = useCallback((x: number, z: number) => {
    // Check map bounds
  ...
  const placeTower = useCallback((x: number, z: number, type: TowerType) => {
  ...
  return (
    <GameContext.Provider value={{ 
      gameState, enemies, towers, projectiles, effects,
      placeTower, startGame, resetGame, 
      selectedTower, setSelectedTower, 
      selectedEntityId, setSelectedEntityId, upgradeTower, sellTower,
      isValidPlacement,
      setEnemies, setTowers, setProjectiles, setEffects, setGameState
    }}>
      {children}
