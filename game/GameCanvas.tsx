...
import { OrbitControls, useCursor, SoftShadows, Line, Ring, Trail, Instance, Instances } from '@react-three/drei';
import * as THREE from 'three';
import { useGame } from './GameState';
import { MAP_GRID, TILE_SIZE, COLORS, TOWER_CONFIGS, MAP_WIDTH, MAP_HEIGHT, PATH_WAYPOINTS, ENEMY_TYPES } from '../constants';
import { TileType, TowerType, EnemyEntity, TowerEntity, ProjectileEntity, EffectEntity } from '../types';
...
const GameLoopBridge = () => {
  const { gameState, enemies, towers, projectiles, 
          setEnemies, setProjectiles, setTowers, setGameState, setEffects
        } = useGame();
  ...
  useFrame((state, delta) => {
    ...
    setEnemies((prev) => {
       ...
       const next = prev.map(e => {
         ...
         if (!p2) { hpLoss++; return null; }

         const totalDist = Math.sqrt(Math.pow(p2[0]-p1[0], 2) + Math.pow(p2[1]-p1[1], 2)) * TILE_SIZE;
         ...
         if (newEnemy.progress >= 1) {
           newEnemy.progress = 0;
           newEnemy.pathIndex++;
           if (newEnemy.pathIndex >= PATH_WAYPOINTS.length - 1) { hpLoss++; return null; }
         }
       ...
       if (hpLoss > 0) {
         setGameState((g) => ({ ...g, lives: Math.max(0, g.lives - hpLoss), gameStatus: g.lives - hpLoss <= 0 ? 'gameover' : g.gameStatus }));
       }
       return next;
    });
    ...
    setTowers((prevTowers) => {
      ...
      const updatedTowers = prevTowers.map(t => {
        ...
        for (const e of enemies) {
           ...
           if (d <= stats.range && d < minDist) { minDist = d; target = e; }
        }
...
const Tile: React.FC<{ x: number, z: number, type: TileType }> = ({ x, z, type }) => {
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial color={color} emissive={color} emissiveIntensity={0.2} roughness={0.1} />
    </mesh>
  );
};
...
const Enemy: React.FC<{ data: EnemyEntity }> = ({ data }) => {
  ...
  return (
    <group position={data.position}>
      ...
      <mesh position={[-0.5 + (data.hp / 100) * 0.5, hpBarY, 0.01]} scale={[Math.max(0, Math.min(1, data.hp / 100)), 1, 1]}>
...
const Tower: React.FC<{ data: TowerEntity, enemies: EnemyEntity[] }> = ({ data, enemies }) => {
  ...
  return (
    ...
    >
      ...
      {/* Turret */}
      <mesh castShadow position={[0, 1, 0]} scale={[1 + (data.level - 1)*0.1, 1 + (data.level - 1)*0.1, 1 + (data.level - 1)*0.1]}>
...
const Projectile: React.FC<{ data: ProjectileEntity, enemies: EnemyEntity[] }> = ({ data, enemies }) => {
...
const Explosion: React.FC<{ data: EffectEntity, remove: (id: string) => void }> = ({ data, remove }) => {
...
const World = () => {
  return (
    <group position={[-MAP_WIDTH * TILE_SIZE / 2 + TILE_SIZE/2, 0, -MAP_HEIGHT * TILE_SIZE / 2 + TILE_SIZE/2]}>
       {/* Ground Grid */}
       {MAP_GRID.map((row, z) => 
         row.map((type, x) => (
           <Tile key={`${x}-${z}`} x={x} z={z} type={type} />
         ))
       )}
    </group>
  );
  ...
  return (
    <>
      ...
      <group position={[offsetX, 0, offsetZ]}>
        {towers.map(t => <Tower key={t.id} data={t} enemies={enemies} />)}
        {enemies.map(e => <Enemy key={e.id} data={e} />)}
        {projectiles.map(p => <Projectile key={p.id} data={p} enemies={enemies} />)}
        {effects.map(e => <Explosion key={e.id} data={e} remove={removeEffect} />)}
      </group>

      <OrbitControls makeDefault minPolarAngle={0} maxPolarAngle={Math.PI / 2.5} minDistance={10} maxDistance={40} />
