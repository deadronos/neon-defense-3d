...
-   [ ] Modify `step` function in `GameState.tsx` to account for `gameSpeed`.
    -   *Technical Decision:* To ensure stability, if `gameSpeed` > 1, we should loop the `stepEngine` call `gameSpeed` times (or similar) with the original delta, OR just scale the delta if the engine is robust.
