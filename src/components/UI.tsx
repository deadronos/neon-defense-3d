    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm z-10 animate-fade-in">
        <div className="text-center p-12 bg-black/40 border-y-2 border-cyan-500/50 relative transform transition-all hover:scale-105 duration-500">
          ...
          <p className="text-cyan-200/70 mb-12 tracking-widest text-sm uppercase">Protocol: Survive // Execute</p>
          <button
            onClick={startGame}
          ...
          >
            ...
            <span className="relative font-bold text-xl tracking-widest text-purple-100 group-hover:text-white">INITIATE</span>
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-red-950/80 backdrop-blur-md z-10 animate-fade-in">
        <div className="text-center p-12 relative border border-red-500/50 bg-black/60">
          <h1 className="text-6xl font-black mb-4 text-red-500 tracking-widest drop-shadow-[0_0_20px_rgba(239,68,68,0.6)]">CRITICAL FAILURE</h1>
          <p className="text-xl mb-8 font-mono text-red-200">WAVES CLEARED: <span className="text-3xl font-bold">{gameState.wave}</span></p>
          <button
            onClick={resetGame}
        ...
        <div className="flex gap-6">
          <div className="relative bg-black/40 border-l-4 border-b border-r border-t border-t-transparent border-r-transparent border-b-transparent border-l-red-500 pl-4 pr-6 py-2 skew-x-[-10deg]">
             <div className="skew-x-[10deg] flex flex-col">
                ...
                <span className={`text-2xl font-bold font-mono ${gameState.lives < 10 ? 'text-red-500 animate-pulse' : 'text-white'}`}>
                    {gameState.lives}%
                </span>
             </div>
          </div>
          ...
          <div className="relative bg-black/40 border-l-4 border-b border-r border-t border-t-transparent border-r-transparent border-b-transparent border-l-yellow-400 pl-4 pr-6 py-2 skew-x-[-10deg]">
             <div className="skew-x-[10deg] flex flex-col">
                ...
                <span className="text-2xl font-bold font-mono text-yellow-300">${gameState.money}</span>
             </div>
          </div>
            ...
            {Object.values(TowerType).map((type) => {
              ...
              return (
                ...
                >
                  ...
                  >
                        <div className="w-6 h-6 rounded-full shadow-[0_0_10px_currentColor]" style={{ backgroundColor: config.color, boxShadow: `0 0 10px ${config.color}`}} />
                  </div>
                  ...
                  <div className="absolute bottom-full mb-6 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-200 w-48 bg-black/95 p-4 text-xs text-left border border-cyan-900 pointer-events-none shadow-2xl z-20 backdrop-blur-md">
                    <h3 className="font-bold text-cyan-400 text-sm mb-1 uppercase tracking-wider border-b border-cyan-900 pb-1">{config.name}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-y-1 text-gray-500 font-mono text-[10px]">
                      <span>DMG: <span className="text-white">{config.damage}</span></span>
                      <span>SPD: <span className="text-white">{(1 / config.cooldown).toFixed(1)}/s</span></span>
                      <span className="col-span-2">RNG: <span className="text-white">{config.range}M</span></span>
                    </div>
                  </div>
                </button>
              );
          ...
          <div className="bg-black/90 backdrop-blur border-t-2 border-cyan-500/50 p-6 flex gap-8 shadow-2xl min-w-[500px] animate-fade-in-up">
            ...
            <div className="flex flex-col gap-2 flex-grow">
              <div className="flex items-center gap-3 border-b border-gray-800 pb-2 mb-2">
                 <div className="w-3 h-3 rounded-full" style={{ background: TOWER_CONFIGS[selectedTowerEntity.type].color, boxShadow: `0 0 10px ${TOWER_CONFIGS[selectedTowerEntity.type].color}` }} />
                 <h2 className="text-xl font-bold text-white uppercase tracking-wider">{TOWER_CONFIGS[selectedTowerEntity.type].name}</h2>
                    ...
                    return (
                        <>
                            <div>
                                ...
                                <div className="font-mono text-white text-lg">{current.damage.toFixed(0)} <span className="text-green-500 text-xs">+{ (next.damage - current.damage).toFixed(0) }</span></div>
                            </div>
                            <div>
                                <div className="text-gray-500 uppercase tracking-widest text-[9px]">Range</div>
                                <div className="font-mono text-white text-lg">{current.range.toFixed(1)} <span className="text-green-500 text-xs">+{ (next.range - current.range).toFixed(1) }</span></div>
                            </div>
                            <div>
                                <div className="text-gray-500 uppercase tracking-widest text-[9px]">Rate</div>
                                <div className="font-mono text-white text-lg">{(1 / current.cooldown).toFixed(1)} <span className="text-green-500 text-xs text-[10px]">UP</span></div>
                            </div>
                        </>
                    )
                 })()}
            ...
            <div className="flex flex-col gap-3 min-w-[140px] border-l border-gray-800 pl-6 justify-center">
                 <button className="text-[10px] text-gray-500 hover:text-white uppercase tracking-widest text-right mb-auto" onClick={() => setSelectedEntityId(null)}>Close [ESC]</button>
                 ...
                 {(() => {
                    ...
                    return (
                         ...
                         >
                            ...
                            <span className={`text-xs font-mono ${canAfford ? 'text-cyan-300' : 'text-gray-500'}`}>${nextStats.upgradeCost}</span>
                         </button>
                    );
                 })()}
