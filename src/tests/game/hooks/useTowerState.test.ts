...
describe('useTowerState', () => {
  ...
  it('validates placement correctly', () => {
    const { result } = renderHook(() => useTowerState(defaultGameState, mockSetGameState, mockMapGrid));
  ...
  it('does not place tower if insufficient money', () => {
      const { result } = renderHook(() => useTowerState({ ...defaultGameState, money: 0 }, mockSetGameState, mockMapGrid));
